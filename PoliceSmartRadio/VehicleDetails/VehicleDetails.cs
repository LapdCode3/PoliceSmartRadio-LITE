    ...
    internal class VehicleDetails
    {
        ...
        public static void AddVehicleToDetailsDatabase(Vehicle veh, int InsuranceChance = 7, int RegistrationChance = 7)
        {
            if (!VehiclesWithDetails.ContainsKey(veh.Handle))
            {
                ...
                if (rnd.Next(InsuranceChance) == 0 && !veh.HasSiren)
                {
                    if (rnd.Next(5) < 2) { InsuranceStatusToAdd = EVehicleDetailsStatus.Expired; }
                ...
                if (rnd.Next(RegistrationChance) == 0 && !veh.HasSiren)
                {
                    if (rnd.Next(5) < 2) { RegistrationStatusToAdd = EVehicleDetailsStatus.Expired; }
